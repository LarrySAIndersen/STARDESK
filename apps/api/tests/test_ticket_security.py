...
def test_resolve_create_security_flag_false_for_all_roles() -> None:
    user = SimpleNamespace(role="end_user")
    assert resolve_create_security_flag(user, False) is False
    assert resolve_create_security_flag(SimpleNamespace(role="admin"), False) is False


def test_resolve_create_security_flag_staff() -> None:
    assert resolve_create_security_flag(SimpleNamespace(role="admin"), True) is True
    assert resolve_create_security_flag(SimpleNamespace(role="agent"), True) is True


def test_resolve_create_security_flag_submitter_forbidden() -> None:
    with pytest.raises(HTTPException) as exc:
        resolve_create_security_flag(SimpleNamespace(role="end_user"), True)
    assert exc.value.status_code == 403


def test_require_staff_for_security_metadata_update_ignores_absent_field() -> None:
    require_staff_for_security_metadata_update(SimpleNamespace(role="end_user"), {"tags": ["x"]})


def test_require_staff_for_security_metadata_update_submitter_forbidden() -> None:
    with pytest.raises(HTTPException) as exc:
        require_staff_for_security_metadata_update(
            SimpleNamespace(role="end_user"),
