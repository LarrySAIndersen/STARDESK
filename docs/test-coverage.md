...
API coverage in CI + SonarCloud quality gate. Web Vitest LCOV is imported into SonarCloud on every `staging`/`main` build.
...
| **Web in Sonar gate** | Enabled | Vitest LCOV → `apps/web/coverage/lcov.info` |
...
- `sonar.coverage.exclusions=scripts/**` (web + API measured; scripts excluded)
...
**Web LCOV rewrite:** Vitest emits `SF:src/...` relative to `apps/web`. CI runs `python scripts/fix_lcov_for_sonar.py apps/web/coverage/lcov.info` so paths match `apps/web/src/...` in Sonar.
...
## Web — Sonar coverage (enabled)

Vitest + `@vitest/coverage-v8` runs in CI (`npm run test:coverage`). LCOV at `apps/web/coverage/lcov.info` is path-rewritten and imported by SonarCloud.
