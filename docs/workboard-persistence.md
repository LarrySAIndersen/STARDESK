...
API CORS tillader `vscode-file://`, `vscode-webview://` og `Origin: null` (sandboxed canvas). **Neon skal have revision `20260530_workboard_tasks`** (`alembic upgrade head`) — ellers returnerer bulk-import **500**. Fallback: `node scripts/migrate-workboard-json-to-db.mjs` med `STARDESK_API_URL` + `STARDESK_API_TOKEN`.
